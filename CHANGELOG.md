...
- Cleaner console output: a short âœ“ progress summary on success, plain-English errors with a suggested fix on failure. Raw HTTP responses and stack traces now only appear on screen with `--debug` (they are still written to `slic3r-debug.txt`)
...
- Multi-filament usage entries now include a note with the slicer slot, basic color name (from `filament_colour`), and material, e.g. `Slot 1 Â· Red (#E72F1D) Â· PLA`, making it easier to match entries to the right spool on 3D Print Log
